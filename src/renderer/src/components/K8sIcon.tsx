...
export default function K8sIcon({ size = 16, className = '', color }: Readonly<{ size?: number; className?: string; color?: string }>) {
  ...
  if (color) {
    return (
      <svg width={size} height={size} viewBox="0 0 48 48" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
        ...
        {spokes.map(s => (
          <line key={s.angle} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} stroke={color} strokeWidth="2.4" strokeLinecap="round" />
        ))}
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {spokes.map(s => (
        <line key={s.angle} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} stroke="white" strokeWidth="2.2" strokeLinecap="round" />
      ))}
      <circle cx="24" cy="24" r="5.5" fill="white" />
      <circle cx="24" cy="24" r="3" fill="#326CE5" />
      {spokes.map(s => (
        <circle key={s.angle} cx={s.tipX} cy={s.tipY} r="2.2" fill="white" />
      ))}
    </svg>
  )
}
